...
- GameStateStore key `__atmosphere.skyless` — game-declared skyless dimension flag (2026-07-09). While set, physical sky/star sampling is suppressed on primary-miss pixels and the composite pass forces full fog opacity against sky, without modifying any RTX option values. See [`docs/RemixAtmosphereAPI.md`](RemixAtmosphereAPI.md).
...
- `remixapi_MaterialInfoOpaqueSpecularEXT`, an opt-in opaque-material pNext
  extension for RGB dielectric specular-F0 textures/constants. Existing
  metallic workflow behavior remains unchanged when the extension is absent.
